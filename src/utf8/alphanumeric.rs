...
mod tests {
    ...
    fn test_unicode_letters() {
        ...
        for (input, expected) in test_cases {
            ...
            assert_eq!(ch, expected, "Failed for Unicode letter: {} (U+{:04X})", input, expected as u32);
        }
    }
    ...
    fn test_non_alphanumeric_fail() {
        ...
        for input in non_alphanumeric {
            ...
            assert!(result.is_err(), "Expected error for non-alphanumeric: {}", input);
            assert!(result.unwrap_err().to_string().contains("expected Unicode letter or digit"), 

...
pub fn unicode_letter() -> impl for<'a> Parser<'a, Output = char> {
    char().filter(
        |c| c.is_alphabetic(), 
        "expected Unicode letter"
    )
}
...
mod tests {
    ...
    fn test_latin_extended_letters() {
        ...
        for (input, expected) in test_cases {
            ...
            let (ch, _) = parser.parse(cursor).unwrap();
            assert_eq!(ch, expected, "Failed for Latin extended: {} (U+{:04X})", input, expected as u32);
        }
    }
    ...
    fn test_greek_letters() {
        ...
        for (input, expected) in test_cases {
            ...
            let (ch, _) = parser.parse(cursor).unwrap();
            assert_eq!(ch, expected, "Failed for Greek letter: {} (U+{:04X})", input, expected as u32);
        }
    }
    ...
    fn test_cyrillic_letters() {
        ...
        for (input, expected) in test_cases {
            ...
            let (ch, _) = parser.parse(cursor).unwrap();
            assert_eq!(ch, expected, "Failed for Cyrillic letter: {} (U+{:04X})", input, expected as u32);
        }
    }
    ...
    fn test_cjk_letters() {
        ...
        for (input, expected) in test_cases {
            ...
            let (ch, _) = parser.parse(cursor).unwrap();
            assert_eq!(ch, expected, "Failed for CJK letter: {} (U+{:04X})", input, expected as u32);
        }
    }
    ...
    fn test_non_letters_fail() {
        ...
        for input in non_letters {
            ...
            assert!(result.unwrap_err().to_string().contains("expected Unicode letter"), 
